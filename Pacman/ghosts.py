...
pygame.init()
intermission_sound = pygame.mixer.Sound("sound/pacman_intermission.wav")
    ...
    def freightMode(self):
        if self.mode.name != "SPAWN" and self.mode.name != "GUIDE":
            if self.mode.name != "FREIGHT":
                ...
                self.mode = Mode("FREIGHT", time=50, speedMult=0.5)
                self.modeTimer = 0
            else:
                self.mode = Mode("FREIGHT", time=50, speedMult=0.5)
                self.modeTimer = 0
...
class GhostGroup(object):
    ...
    def freightMode(self):
        ...
        pygame.mixer.Sound.play(intermission_sound)
        for ghost in self:
            ghost.freightMode()
